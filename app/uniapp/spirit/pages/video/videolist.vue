	...
	<view class="container">
			...
			<view v-if="showFilter" class="filter-panel">
				<view class="filter-row">
					...
					<input class="filter-input" v-model="selectedPlatform" placeholder="输入平台名称..."
						placeholder-class="placeholder" @input="onPlatformInput" />
				</view>

				<view class="filter-row">
					<text class="filter-label">视频标签</text>
					<input class="filter-input" v-model="selectedTag" placeholder="输入标签名称..."
						placeholder-class="placeholder" @input="onTagInput" />
				</view>
		...
		<view class="content-wrap">
			...
			<view class="video-list">
				<!-- 骨架屏卡片 - 只在加载中且列表为空时显示 -->
				<view class="video-card skeleton-card" v-for="n in 6" :key="n" v-if="isLoading && list.length === 0">
				...
				<view class="video-card" v-for="(item, index) in list" :key="index"
					...
					<view class="video-card-inner">
						<view class="video-cover" @click="togglePlayBtn(index)" @touchstart="showPlayButton(index)"
	...
	export default {
		...
		methods: {
			normalizePath(rawPath,server,port,token) {
				//如果小程序端或者app端缩略图 显示有问题 这里直接修改原来方式拼接  这里可能会因为跨平台问题 造成符号不一样
				if (!rawPath) return '';
